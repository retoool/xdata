...
export class UserApi {
  ...
  static async getUserList(
    ...
    return http.request<PageResult<User>>("GET", "/system/user/list", {
      params
    });
  }
  ...
  static async batchDeleteUsers(ids: number[]): Promise<void> {
    return http.request<void>("POST", "/system/user/batch-delete", {
      data: { ids }
    });
  }
  ...
  static async updateUserStatus(id: number, status: number): Promise<void> {
    return http.request<void>("POST", `/system/user/${id}/status`, {
      data: { status }
    });
  }
  ...
  static async uploadAvatar(file: File): Promise<string> {
    ...
    return http.request<string>("POST", "/system/user/upload-avatar", {
      data: formData
    });
  }
  ...
  static async updateUserAvatar(id: number, avatar: string): Promise<void> {
    return http.request<void>("POST", `/system/user/${id}/avatar`, {
      data: { avatar }
    });
  }
  ...
  static async assignUserRoles(id: number, roleIds: number[]): Promise<void> {
    return http.request<void>("POST", `/system/user/${id}/roles`, {
      data: { roleIds }
    });
  }
