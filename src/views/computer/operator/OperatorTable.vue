  ...
  <div class="operator-table-container">
        ...
        <el-table-column prop="type" label="算子类型" width="100">
          <template #default="{ row }">
            <el-tag
              ...
              :title="`右键查看其他算子类型`"
              @contextmenu.prevent="
                handleOperatorTypeRightClick($event, row.type)
              "
            >
              ...
              >
                <span
                  ...
                  :class="{
                    ...
                    'category-unselected': !isCategorySelected(
                      row.categoryPath,
                      index
                    )
                  }"
                  ...
                  @contextmenu.prevent="
                    handleCategoryRightClick($event, row.categoryPath, index)
                  "
                >
                  {{ category }}
                </span>
                <span
                  v-if="index < row.categoryPath.length - 1"
    ...
    >
      ...
      <div
        ...
        :class="{
          'menu-item-active':
            category === currentCategoryPath[currentCategoryIndex]
        }"
...
const formRules: FormRules = {
  ...
  type: [{ required: true, message: "请选择算子类型", trigger: "change" }],
  description: [{ required: true, message: "请输入算子描述", trigger: "blur" }],
...
const handleBatchDelete = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      `确定要删除选中的 ${selectedRows.value.length} 个算子吗？`,
      "提示",
      {
        type: "warning"
      }
...
const handleCategoryClick = (categoryPath: string[], clickIndex: number) => {
  try {
    ...
    ElMessage.error(
      `分类跳转失败: ${error instanceof Error ? error.message : "未知错误"}`
    );
  }
};
...
// 显示同级分类菜单
const showSiblingCategoriesMenu = (
  event: MouseEvent,
  categoryPath: string[],
  clickIndex: number
) => {
...
const getSiblingCategories = (
  ...
  if (
    ...
    clickIndex < 0 ||
    clickIndex >= categoryPath.length
  ) {
    return [];
  }
...
const selectSiblingCategory = (categoryName: string) => {
  try {
    ...
    if (
      ...
      !Array.isArray(currentCategoryPath.value)
    ) {
    ...
    if (
      ...
      currentCategoryIndex.value >= currentCategoryPath.value.length
    ) {
    ...
    // 创建新的分类路径
    const newPath = [
      ...currentCategoryPath.value.slice(0, currentCategoryIndex.value + 1)
    ];
    newPath[currentCategoryIndex.value] = categoryName;

    // 发出事件通知父组件进行分类切换
    debugEmit("category-click", {
      path: newPath,
      index: currentCategoryIndex.value
    });
    ...
    ElMessage.error(
      `切换分类失败: ${error instanceof Error ? error.message : "未知错误"}`
    );
  }
};
...
// 处理算子类型右键点击
const handleOperatorTypeRightClick = (
  event: MouseEvent,
...
const calculateTextWidth = (text: string): number => {
  ...
  // 设置字体样式，与CSS中的字体保持一致
  context.font =
    '14px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, "Noto Sans", sans-serif';
