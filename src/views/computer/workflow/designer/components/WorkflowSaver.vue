<template>
  <div class="workflow-saver">
    ...
    <!-- 发布确认对话框 -->
    <el-dialog v-model="publishDialogVisible" title="发布工作流" width="600px">
...
import { Loading, Check, Close, CircleCheck } from "@element-plus/icons-vue";
...
const validateWorkflow = async () => {
  ...
  const nodeErrors = props.nodes.filter(
    node => node.data.params?.some(param => param.error) || false
  );
  ...
  const isolatedNodes = props.nodes.filter(
    node => !connectedNodeIds.has(node.id) && props.nodes.length > 1
  );
