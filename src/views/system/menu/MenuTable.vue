  ...
  <div class="menu-table">
          ...
          <template #default="{ row }">
            <div class="action-buttons">
              ...
              <el-dropdown trigger="click">
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    <el-dropdown-item @click.stop="handleAddChild(row)">
                      <el-icon style="margin-right: 4px;"><Plus /></el-icon> 新增
                    </el-dropdown-item>
                    <el-dropdown-item @click.stop="handleCopy(row)">
                      <el-icon style="margin-right: 4px;"><CopyDocument /></el-icon> 复制
                    </el-dropdown-item>
                    <el-dropdown-item @click.stop="handleMove(row)">
                      <el-icon style="margin-right: 4px;"><Position /></el-icon> 移动
                    </el-dropdown-item>
                    <el-dropdown-item 
                      :disabled="(row.children && row.children.length > 0) || row.id <= 5"
                      @click.stop="handleDelete(row)"
                      :title="row.children && row.children.length > 0 ? '请先删除子菜单' : (row.id <= 5 ? '系统菜单不能删除' : '')"
    ...
    >
      <div class="move-dialog-content">
        <div class="move-info">
          <p>将 <strong>{{ moveMenu?.title }}</strong> ({{ getMenuTypeText(moveMenu?.type) }}) 移动到：</p>
          <div class="move-rules">
            <p class="rules-title">移动规则：</p>
            <ul class="rules-list">
              <li v-if="moveMenu?.type === 1">• 目录只能移动到其他目录下</li>
              <li v-if="moveMenu?.type === 2">• 菜单只能移动到目录下</li>
              <li v-if="moveMenu?.type === 3">• 按钮只能移动到菜单下</li>
              <li>• 不能移动到自己的子菜单下</li>
            </ul>
          </div>
        </div>
        <div class="move-layout">
          ...
          <div class="target-children">
            <h4>
              ...
              <span v-if="targetMenuId !== null && targetMenuId !== undefined" class="menu-count">
                ({{ targetChildrenList.length }}项)
              </span>
              <span v-if="targetMenuId === 0 && moveMenu" class="current-status">
                - 当前显示根目录下的所有菜单
              </span>
            </h4>
            <div class="children-card">
              <div v-if="targetMenuId === null || targetMenuId === undefined" class="no-selection">
              ...
              <div v-else class="children-list">
                ...
                >
                  <template #item="{ element, index }">
                    <div class="children-item" :class="{ 'is-moving': element.id === moveMenu?.id }">
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button type="primary" @click="confirmMove" :loading="moveLoading">
            确认移动
          </el-button>
        </div>
      </template>
...
const confirmMove = async () => {
  if (!moveMenu.value || targetMenuId.value === null || targetMenuId.value === undefined) {
  ...
  try {
    // 使用专门的moveMenu API
    await moveMenuApi(moveMenu.value.id, targetMenuId.value === 0 ? null : targetMenuId.value)
...
const updateTargetChildrenList = () => {
  if (targetMenuId.value === null || targetMenuId.value === undefined || !moveMenu.value) {
    targetChildrenList.value = []
    originalChildrenList.value = []
    hasChildrenChanged.value = false
    return
  ...
  if (targetMenuId.value === 0) {
    // 根目录情况：获取所有第一级菜单
    children = tableData.value.filter(menu => !menu.parentId || menu.parentId === 0)
  } else {
    // 普通父级菜单情况
  ...
  // 检查要移动的菜单是否已经在当前列表中
  const existingIndex = children.findIndex(child => child.id === moveMenu.value.id)
...
const dragEnd = () => {
  ...
  // 检查顺序是否改变
  hasChildrenChanged.value = JSON.stringify(targetChildrenList.value) !== JSON.stringify(originalChildrenList.value)
}
...
const initCurrentParentMenu = () => {
  ...
  // 检查是否为第一级菜单（根目录下的菜单）
  const isFirstLevelMenu = !moveMenu.value.parentId || moveMenu.value.parentId === 0
  ...
  if (isFirstLevelMenu) {
    ...
    // 非第一级菜单：找到当前菜单的父级菜单
    const findParentMenu = (menus: MenuType[], childId: number): MenuType | null => {
      for (const menu of menus) {
        if (menu.children) {
